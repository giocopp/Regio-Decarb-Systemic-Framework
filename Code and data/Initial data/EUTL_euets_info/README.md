...
Source for the **ETS leg** of the carbon-cost-at-risk Exposure: installation-level verified emissions + free allocation, geocoded to NUTS-2 / NUTS-3.
...
- Manufacturing = EUTL activity codes **21–44** (validated against the official `activity_type.csv`), mapped to C16-C18 / C19-C20 / C23 / C24.
...
The raw zip (`installation.csv` 5 MB, `compliance.csv` 61 MB, `transaction.csv` 164 MB) is not version-controlled. Re-download from the URL above and extract to `/tmp/eutl_validation/` (or edit `RAW` in `prototypes/ets_geocode.R`).
