...
harmonize_non_sector <- function(file_paths, base_data_path) {
  ...
  base_ids <- readxl::read_excel(base_data_path) |>
    janitor::clean_names() |>
    dplyr::filter(nchar(nuts_id) == 4) |>
    dplyr::select(cntr_code, nuts_id, nuts_name) |>
    dplyr::distinct() |>
    dplyr::rename(Country_ID = cntr_code,
           NUTS_ID    = nuts_id,
           NUTS_Name  = nuts_name)
  ...
  read_one <- function(file_path) {
    ...
    title_cell    <- readxl::read_excel(file_path, range = "A1", col_names = FALSE)[[1, 1]]
    has_metadata  <- grepl("Dimension:", title_cell)

    data <- if (has_metadata) readxl::read_excel(file_path, skip = 1) else readxl::read_excel(file_path)
    data <- janitor::clean_names(data)

    # -- extract Dimension & Indicator --
    if (has_metadata) {
      dimension <- stringr::str_extract(title_cell, "(?<=Dimension: )[^;]+") |> stringr::str_trim()
      indicator <- stringr::str_extract(title_cell, "(?<=Indicator: )[^;]+") |> stringr::str_trim()
    } else {
    ...
    if (!any(grepl("nuts.*id", names(data))) && "region_code" %in% names(data)) {
      data <- dplyr::rename(data, nuts_id = region_code)
    } else {
      cand <- names(data)[grepl("nuts.*id", names(data))]
      if (length(cand) > 0) data <- dplyr::rename(data, nuts_id = dplyr::all_of(cand[1]))
      else                       data <- dplyr::mutate(data, nuts_id = NA_character_)
    }

    # NUTS_Name
    nm_cand <- names(data)[grepl("nuts.*name", names(data))]
    if (length(nm_cand) > 0) data <- dplyr::rename(data, nuts_name = dplyr::all_of(nm_cand[1]))
    else                         data <- dplyr::mutate(data, nuts_name = NA_character_)

    # Country_ID
    if ("country_cd" %in% names(data)) {
      data <- dplyr::rename(data, country_id = country_cd)
    } else if (!"country_id" %in% names(data)) {
      data <- dplyr::mutate(data, country_id = dplyr::if_else(!is.na(nuts_id),
                                                 substr(nuts_id, 1, 2),
                                                 NA_character_))
    }
    ...
    if (!"value" %in% names(data)) {
      if ("gfcf" %in% names(data)) data <- dplyr::rename(data, value = gfcf)
      else                              data <- dplyr::mutate(data, value = NA_real_)
    }

    # Unit
    if (!"unit" %in% names(data)) data <- dplyr::mutate(data, unit = NA_character_)
    ...
    data |>
      ...
      dplyr::select(country_id, nuts_id, nuts_name, sector_id, sector_name,
             component, dimension, indicator, unit, value, value_n) |>
      purrr::set_names(c("Country_ID", "NUTS_ID", "NUTS_Name", "Sector_ID", "Sector_Name",
                   "Component", "Dimension", "Indicator", "Unit", "Value", "Value_N"))
  }

  # ── process all files ──
  raw <- purrr::map(file_paths, read_one) |> dplyr::bind_rows()

  # ── harmonise regions against base list ──
  harmonised <- raw |>
    dplyr::select(-Country_ID, -NUTS_Name) |>
    dplyr::left_join(base_ids, by = "NUTS_ID") |>
    dplyr::select(Country_ID, NUTS_ID, NUTS_Name, dplyr::everything())

  # ── complete grid: every region x every indicator ──
  indicators_info <- harmonised |>
    dplyr::select(Indicator, Dimension, Component, Unit) |>
    dplyr::distinct()

  out <- tidyr::crossing(base_ids, indicators_info) |>
    dplyr::left_join(harmonised,
  ...
  out <- out |>
    dplyr::mutate(
      Indicator = dplyr::if_else(Indicator == "0 Summary Innovation Index",
    ...
    dplyr::filter(!(Dimension == "Technology" & is.na(Indicator))) |>
...
harmonize_sector <- function(file_paths, non_sector_data, empl_weights) {
  ...
  read_one <- function(file_path) {
    ...
    data <- readxl::read_excel(file_path) |> janitor::clean_names()

    # -- handle value column --
    if (!"value" %in% names(data)) {
      if (all(c("tang_inv", "intang_inv") %in% names(data))) {
        data <- data |>
          tidyr::pivot_longer(cols = c(tang_inv, intang_inv),
                       names_to = "value_type", values_to = "value") |>
          dplyr::mutate(value_n = NA_real_)
        if (!"indicator" %in% names(data)) {
          data <- dplyr::mutate(data, indicator = value_type)
        }
      } else {
        data <- dplyr::mutate(data, value = NA_real_, value_n = NA_real_)
      }
    } else {
      if (!"value_n" %in% names(data)) data <- dplyr::mutate(data, value_n = NA_real_)
    }
  ...
  sector_data <- purrr::map(file_paths, read_one) |> dplyr::bind_rows()

  # ── fill NUTS_Name from non-sector region mapping ──
  region_mapping <- non_sector_data |>
    dplyr::select(Country_ID, NUTS_ID, NUTS_Name) |>
    dplyr::distinct()

  sector_data <- sector_data |>
    dplyr::left_join(region_mapping, by = c("Country_ID", "NUTS_ID"),
              suffix = c("", ".ns")) |>
    dplyr::mutate(NUTS_Name = dplyr::coalesce(NUTS_Name, NUTS_Name.ns)) |>
    dplyr::select(-NUTS_Name.ns)

  # ── collapse C31-C32 + C33 into C31-C33 ──
  sector_data <- sector_data |>
    dplyr::mutate(Sector_ID = dplyr::if_else(Sector_ID %in% c("C31-C32", "C33"),
                               "C31-C33", Sector_ID)) |>
    dplyr::group_by(Country_ID, NUTS_ID, NUTS_Name, Sector_ID, Sector_Name,
             Component, Dimension, Indicator, Unit) |>
    dplyr::summarise(Value   = if (all(is.na(Value))) NA_real_
...
combine_all <- function(sector_data, non_sector_data) {
  ...
  sector_data <- sector_data |>
    dplyr::filter(!is.na(Sector_ID), !Sector_ID %in% c("A", "H"))

  all_data <- dplyr::bind_rows(
    sector_data     |> dplyr::select(-Value_N),
    non_sector_data |> dplyr::select(-Value_N)
  )

  # Fill Country_ID from NUTS_ID where missing
  all_data <- all_data |>
    dplyr::mutate(Country_ID = dplyr::if_else(
  ...
  all_data <- all_data |>
    ...
    tidyr::fill(NUTS_Name, .direction = "downup") |>
    dplyr::ungroup()

  # Set NUTS_ID / NUTS_Name to NA when they equal the country code
  all_data <- all_data |>
    dplyr::mutate(
      NUTS_ID   = dplyr::if_else(NUTS_ID == Country_ID, NA_character_, NUTS_ID),
      NUTS_Name = dplyr::if_else(NUTS_Name == NUTS_ID,  NA_character_, NUTS_Name)
    )
