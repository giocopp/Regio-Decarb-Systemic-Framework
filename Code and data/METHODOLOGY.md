...
Exposure is the **covered carbon per employee of the sector's regional workforce** (FINAL, 2026-07-09; `tri_exposure_denom = "per_employee"`, `tri_norm_mode = "log"`): the cell's ETS verified emissions plus the embodied carbon in its extra-EU imports of CBAM-covered goods, divided by the cell's sector employment, log-scaled:
...
$$\text{Exposure}_{r,s} = \text{range01}\!\left(\log\!\left(1 + \frac{\text{Exposure\_raw}_{r,s}}{L_{r,s}}\right)\right) \quad \text{POOLED across all } (r,s) \text{ cells}$$

with $L_{r,s}$ the cell's sector employment (`sbs_r_nuts2021`). The intensity form removes the size component; the sector-level denominator keeps the construct at the sector's own jobs — the methodologically correct unit: those jobs are what the transition exposes. The documented imprecision of the employment source (below) is a limitation of official statistics, disclosed and mitigated, not a reason to change the construct. The raw tonnes remain published (`exposure_ETS`, `exposure_CBAM`, `exposure_total`) next to the intensity (`exposure_per_empl`, `empl_int` = $L$ as used, including fallbacks).

**Denominator fallbacks.** Where cell employment is confidential or zero but covered carbon exists, $L$ falls back to (tier 2) the region's manufacturing employment × the sector's national employment share (e.g. NL32 C24 — IJmuiden), then (tier 3) the region's manufacturing employment × the sector's EU-wide employment share (LU C24, CY C26-C27). Zero-carbon cells stay Exposure = 0 regardless.

**Why log.** NUTS-2 sector employment fails in four verified ways (headquarters attribution — EL65 books 43 refining employees against the ~2 Mt Corinth refinery while 80% of Greek C19 employment sits in Attiki; confidentiality suppression as missing rows — NL32 C24, ES62 C19; the SBS "persons employed" definition excludes supplied manpower, so outsourced plants report skeleton payrolls; and reporting volatility, e.g. PT11 C19 462 → 55 in one year). All four shrink denominators at plant regions and inflate intensities there. Under a linear pooled min-max the single most extreme artifact (EL65, ~18,000 t/employee) pinned the scale and 96% of positive cells scored < 0.05; the **log transform** compresses the artifact outliers so the scale is informative again (96% of Total-Manufacturing regions in [0.2, 0.8]; the JRC-handbook-standard treatment of right-skewed indicators). **Disclosed residual:** log is monotone, so the artifact cells keep their (inflated) *positions* — the highest-ranked cell in the panel is EL65 × C19-C20 itself, and five further cells carry > 0.5 Mt against < 1,000 reported employees (EL63/EL64 C23, EL64 C24, SK01 C19-C20, ITF2 C23). These cells are identified here rather than corrected: correcting them would require employment data the EU statistical system does not publish.

**Sensitivity rows (§14).** Normalisation: log vs min-max ρ ≈ 0.78, log vs rank ρ ≈ 0.98 (the Exposure *ordering* is identical under all three — only spacing changes). Denominator: vs raw volume (no division) ρ ≈ 0.75; vs per-regional-GDP ρ ≈ 0.80. CBAM allocation (employment vs plant-emission hybrid) ρ ≈ 0.97; CBAM intensity (direct vs embodied) ρ ≈ 0.99. Maps use quintile classes of the plotted values (display only).
