...
Two extensive indicators are converted to per-employee intensity before min-max normalisation, in `R/04_normalize.R` (`to_per_empl`): **Gross_Fixed_Capital_Formation** and **BERD**. The division uses the NUTS-2 × Sector manufacturing employment count from `empl_weights`.
...
- $Q^{CBAM}$: embodied direct carbon in **extra-EU imports of covered goods** (FIGARO MRIO; origin industries C20/C23/C24 as the covered-goods proxy), charged to **every importing manufacturing sector** — CBAM falls on whoever imports covered inputs, which is why all 11 sub-sectors carry exposure. Caveat: FIGARO 2-digit industries are broader than the exact CBAM goods list and the full EUA price is applied to all embodied import carbon, so the CBAM leg is an upper bound on incidence.
...
- **`carbon_cost_tri`** — `run_sensitivity_cost()` (`R/exposure_cost.R`): the headline cost TRI vs the emissions baseline, vs its `minmax` and `rank` normalisation variants, and vs the 2024 policy state (current free allocation, no CBAM).
